  ...
  journey: [
    {
      ...
      policyEvidence: ["Training Standard \xA72.1", "Secure SDLC Policy \xA73.1"],
      actions: [
    ...
    },
    {
      ...
      policyEvidence: ["Joiner Control Standard \xA71.2"],
    ...
    },
    {
      ...
      policyEvidence: ["Developer Access Standard \xA74.2", "Secure SDLC Policy \xA73.1"],
      actions: [
    ...
    },
    {
      ...
      policyEvidence: ["Collaboration Access Baseline \xA72.4"],
    ...
    },
    {
      ...
      policyEvidence: ["Application Onboarding Checklist \xA71.1"],
      actions: [
    ...
    },
    {
      ...
      policyEvidence: ["Workplace Setup Standard \xA75.1"],
      actions: [
    ...
    },
    {
      ...
      policyEvidence: ["Production Access Policy v2.3 \xA74.1", "Privileged Access Standard \xA72.6"],
      actions: [
    ...
    },
    {
      ...
      policyEvidence: ["Data Access Control \xA73.3"],
    ...
    },
    {
      ...
      policyEvidence: ["Engineering Onboarding Guide \xA72.2"],
    ...
    },
    {
      ...
      policyEvidence: ["Joiner Evidence Standard \xA76.1"],
...
function cloneCase() {
  return JSON.parse(JSON.stringify(MOCK_ONBOARDING_CASE));
}
var OnboardingStateService = class _OnboardingStateService {
  caseSignal = signal(cloneCase(), ...ngDevMode ? [{ debugName: "caseSignal" }] : []);
  caseState = computed(() => this.caseSignal(), ...ngDevMode ? [{ debugName: "caseState" }] : []);
  ...
  }, ...ngDevMode ? [{ debugName: "dashboardSummary" }] : []);
  resetDemo() {
    this.caseSignal.set(cloneCase());
  }
  analyseProfile() {
    this.caseSignal.update((state) => __spreadProps(__spreadValues({}, state), {
      profileAnalysed: true,
      profileConfidence: 94,
  ...
  completeTraining(trainingId) {
    this.caseSignal.update((state) => {
      const trainings = state.trainings.map((training) => training.id === trainingId ? __spreadProps(__spreadValues({}, training), { status: "Completed" }) : training);
      const readiness = __spreadProps(__spreadValues({}, state.readiness), {
  ...
  assignReviewer() {
    this.caseSignal.update((state) => {
      const missingInformation = state.missingInformation.map((item) => item.id === "access-reviewer" ? __spreadProps(__spreadValues({}, item), { resolved: true }) : item);
      const journey = state.journey.map((step) => {
        if (step.key !== "github-access")
          return step;
        return __spreadProps(__spreadValues({}, step), {
          actions: step.actions.map((action) => action.id === "a6" ? __spreadProps(__spreadValues({}, action), { completed: true }) : action),
  ...
  approveAccess(requestId) {
    this.caseSignal.update((state) => {
      const accessRequests = state.accessRequests.map((request) => request.id === requestId ? __spreadProps(__spreadValues({}, request), { status: "Approved" }) : request);
      const journey = state.journey.map((step) => {
        if (requestId === "ar-1" && step.key === "github-access") {
          return __spreadProps(__spreadValues({}, step), {
            status: "Completed",
            actions: step.actions.map((action) => __spreadProps(__spreadValues({}, action), { completed: true })),
        ...
        if (requestId === "ar-1" && step.key === "application-onboarding") {
          return __spreadProps(__spreadValues({}, step), { status: "In Progress" });
        }
        return step;
      });
      const tasks = state.tasks.map((task) => task.title.toLowerCase().includes("github") ? __spreadProps(__spreadValues({}, task), { completed: true }) : task);
      const readiness = __spreadProps(__spreadValues({}, state.readiness), {
        overall: 86,
        access: 85,
  ...
  requestMoreInfo(requestId) {
    this.caseSignal.update((state) => {
      const accessRequests = state.accessRequests.map((request) => request.id === requestId ? __spreadProps(__spreadValues({}, request), { status: "Needs Info" }) : request);
      return __spreadProps(__spreadValues({}, state), {
        accessRequests,
  ...
  answerPolicyQuestion(question) {
    ...
    if (q.includes("production") || q.includes("privileged")) {
      return {
        ...
        sources: ["Production Access Policy v2.3 \xA74.1", "Privileged Access Standard \xA72.6"]
      };
    }
    if (q.includes("jira")) {
      return {
        ...
        sources: ["Collaboration Access Baseline \xA72.4"]
      };
    }
    if (q.includes("training")) {
      return {
        ...
        sources: ["Training Standard \xA72.1", "Secure SDLC Policy \xA73.1"]
      };
    }
    return {
      ...
      sources: ["Joiner Control Standard \xA71.2", "Responsible AI Control Note \xA75.1"]
    };
  }
  activity(title, description, actor, type) {
    return {
      id: `act-${Date.now()}`,
      title,
      description,
      actor,
      type,
      timestamp: (/* @__PURE__ */ new Date()).toLocaleString("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })
    };
  }
  static \u0275fac = function OnboardingStateService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _OnboardingStateService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _OnboardingStateService, factory: _OnboardingStateService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(OnboardingStateService, [{
    type: Injectable,
...
//# sourceMappingURL=chunk-CIBLNRIR.js.map
